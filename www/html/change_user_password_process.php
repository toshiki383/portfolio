<?php
// それぞれのページから関数を取得
require_once '../conf/const.php';
require_once MODEL_PATH . 'functions.php';
require_once MODEL_PATH . 'user.php';

// セッションを開始
session_start();

// iframe対策
header("X-FRAME-OPTIONS: DENY");

// ログインされていなければログインページに戻る
if(is_logined() === false){
    redirect_to(LOGIN_URL);
}

// データベースに接続
$db = get_db_connect();

// ユーザー情報を取得
$acount = get_login_user($db);

// POSTから情報を取得
$old_password = get_post('old_password');
$new_password = get_post('new_password');
$password_confirmation = get_post('password_confirmation');

// ユーザ情報からパスワードを取得
$select_password = $acount['password'];
$select_id = $acount['user_id'];

// POSTからトークンを取得
$token = get_post('csrf_token');

// sessionからトークンを取得
$session = get_session('csrf_token');

// トークンが適正であれば以下を実行
if(is_valid_csrf_token($token, $session) === true){
    
    try{
        $update = change_user_password($db, $select_id, $select_password, $old_password, $new_password, $password_confirmation);

        // パスワードの変更
        if($update === false){
            set_error('パスワードの変更に失敗しました。');
            // マイページに戻る
            redirect_to(PASSWORD_URL);
        }
    }catch(PDOException $e){
    set_error('パスワードの変更に失敗しました。');
    redirect_to(PASSWORD_URL);
    }

    set_message('パスワードを変更しました。');

    // マイページに戻る
    redirect_to(MYPAGE_URL);

// 不正アクセスの場合
}else{
  // エラーを表示
  set_error('不正なアクセスです。');

  // ホームページに戻る
  redirect_to(LOGIN_URL);
}